...
void setLeftInputs(bool dirHigh, bool pwmHigh) {
  digitalWrite(LEFT_DIR_PIN, dirHigh ? HIGH : LOW);
  digitalWrite(LEFT_PWM_PIN, pwmHigh ? HIGH : LOW);
}

void stopAll() {
  setLeftInputs(false, false);
...
void startTest(uint8_t state) {
  ...
  stopAll();
  setLeftInputs(dirHigh, pwmHigh);
  pulseStartedAt = millis();
  pulseActive = true;

  Serial.print(F("LEFT DIR="));
...
void setup() {
  ...
  Serial.println(F("LEFT MOTOR TRUTH TABLE READY: send 0,1,2,3"));
}
...
void loop() {
  ...
  if (pulseActive &&
    ...
    stopAll();
    pulseActive = false;
    Serial.println(F("LEFT SAFE DIR=0 PWM=0"));
  }
}
